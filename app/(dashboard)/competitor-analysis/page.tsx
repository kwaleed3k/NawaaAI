...
function ScoreBar({ score, color, height = "h-3" }: { score: number; color?: string; height?: string }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-sm font-bold font-mono w-10 text-right text-[#2d3142]">{score}</span>
    </div>
  );
}
...
function ScoreCircle({ score, size = 80, label }: { score: number; size?: number; label?: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      {label && <span className="text-sm font-bold text-[#8f96a3] text-center">{label}</span>}
    </div>
  );
}
...
function SectionCard({ children, className = "", accentColor }: { children: React.ReactNode; className?: string; accentColor?: string }) {
  return (
    <div className={`rounded-xl border border-white/20 p-8 relative overflow-hidden ${className}`} style={{ background: "rgba(255,255,255,0.9)", backdropFilter: "blur(12px)" }}>
...
function SectionTitle({ icon: Icon, children, color = "text-[#23ab7e]" }: { icon: React.ComponentType<{ className?: string }>; children: React.ReactNode; color?: string }) {
  return <h3 className={`flex items-center gap-3 text-xl font-extrabold text-[#2d3142] mb-5`}><Icon className={`h-6 w-6 ${color}`} />{children}</h3>;
}
  ...
  return (
    <div className="space-y-8 w-full" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="relative overflow-hidden rounded-xl nl-aurora-bg p-4 sm:p-5 lg:p-6">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-2 text-sm text-white/60">{t.pageSub}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-end gap-3 flex-wrap">
        {analysisData && (
          <>
            <button onClick={handleExportPdf} className="flex items-center gap-2 px-5 py-3 rounded-xl border-2 border-[#e8eaef] text-sm font-bold text-[#505868] hover:bg-[#f4f6f8] transition-colors">
              <Download className="h-5 w-5" /> {t.exportPdf}
            </button>
            <button onClick={saveAnalysis} disabled={saving} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-[#23ab7e] to-[#8054b8] text-sm font-bold text-white shadow-md hover:shadow-lg transition-all">
              {saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <Save className="h-5 w-5" />}
              {saveStatus || t.save}
            </button>
          </>
        )}
        <button onClick={() => setShowPrevious(!showPrevious)} className="flex items-center gap-2 px-5 py-3 rounded-xl border-2 border-[#e8eaef] text-sm font-bold text-[#505868] hover:bg-[#f4f6f8] transition-colors">
      ...
      {showPrevious && (
        <SectionCard>
          <h3 className="font-extrabold text-xl text-[#2d3142] mb-4">{t.loadPrevious}</h3>
          {savedAnalyses.length === 0 ? <p className="text-sm text-[#8f96a3]">{t.noPreviousAnalyses}</p> : (
            <div className="space-y-3">{savedAnalyses.map((sa) => (
              <div key={sa.id} className="flex items-center justify-between p-4 rounded-xl border-2 border-[#e8eaef] hover:bg-[#f4f6f8] transition-colors">
                <div><p className="text-sm font-bold text-[#2d3142]">{sa.competitors?.map((c) => c.name).join(", ")}</p><p className="text-sm text-[#8f96a3]">{new Date(sa.created_at).toLocaleDateString()}</p></div>
                <div className="flex gap-2">
                  <button onClick={() => loadAnalysis(sa)} className="px-4 py-2 rounded-xl border-2 border-[#23ab7e] text-sm font-bold text-[#23ab7e] hover:bg-[#f4f6f8]">{t.load}</button>
      ...
      {/* ═══ COMPANY SELECTOR ═══ */}
      <div className="rounded-xl border border-white/20 p-8" style={{ background: "rgba(255,255,255,0.9)", backdropFilter: "blur(12px)" }}>
        <div className="flex items-center gap-3 mb-5">
          ...
          <h3 className="font-extrabold text-xl text-[#2d3142]">{locale === "ar" ? "اختر الشركة" : "Select Company"}</h3>
        </div>
        {companies.length === 0 ? (
          <p className="text-sm text-[#8f96a3]">{locale === "ar" ? "لا توجد شركات. أضف شركة أولاً." : "No companies found. Add one from Companies page."}</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {companies.map((c) => (
              <button key={c.id} onClick={() => setSelectedCompany(c)} className={`p-5 rounded-xl border-2 text-start transition-all ${selectedCompany?.id === c.id ? "border-[#23ab7e] bg-[#f4f6f8] shadow-[0_0_0_4px_rgba(35,171,126,0.1)]" : "border-[#e8eaef] hover:border-[#8054b8] hover:bg-[#fafbfd]"}`}>
                <p className="font-bold text-xl text-[#2d3142] truncate">{c.name}</p>
                {c.industry && <p className="text-sm text-[#8f96a3] mt-1">{c.industry}</p>}
              </button>
            ))}
          </div>
        )}
      ...
      {/* ═══ COMPETITOR INPUT ═══ */}
      <div className="rounded-xl border border-white/20 p-8 space-y-5" style={{ background: "rgba(255,255,255,0.9)", backdropFilter: "blur(12px)" }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-xl text-[#2d3142]">{t.addCompetitor}</h3>
              <p className="text-sm text-[#8f96a3]">{t.maxCompetitors} · {competitors.length}/5</p>
            </div>
          </div>
          <button onClick={addCompetitor} disabled={competitors.length >= 5} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 border-[#e8eaef] text-sm font-bold text-[#23ab7e] hover:bg-[#f4f6f8] disabled:opacity-40 transition-colors">
            <Plus className="h-5 w-5" /> {t.addCompetitor}
          </button>
        </div>

        {competitors.map((comp, idx) => (
          <div key={idx} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_160px_1fr_auto] gap-4 items-end p-3 rounded-xl border-2 border-[#e8eaef] bg-[#fafbfd]">
            <div>
              <label className="text-sm font-bold text-[#8f96a3] mb-1.5 block">{t.competitorName}</label>
              <input value={comp.name} onChange={(e) => updateCompetitor(idx, "name", e.target.value)} placeholder={locale === "ar" ? "اسم المنافس" : "Competitor name"} className="w-full h-10 rounded-xl border-2 border-[#e8eaef] bg-white px-4 text-sm text-[#2d3142] focus:border-[#23ab7e] focus:outline-none transition-colors" />
            </div>
            <div>
              <label className="text-sm font-bold text-[#8f96a3] mb-1.5 block">{t.socialHandle}</label>
              <input value={comp.handle} onChange={(e) => updateCompetitor(idx, "handle", e.target.value)} placeholder="@handle" className="w-full h-10 rounded-xl border-2 border-[#e8eaef] bg-white px-4 text-sm text-[#2d3142] focus:border-[#23ab7e] focus:outline-none transition-colors" />
            </div>
            <div>
              <label className="text-sm font-bold text-[#8f96a3] mb-1.5 block">{t.platform}</label>
              <select value={comp.platform} onChange={(e) => updateCompetitor(idx, "platform", e.target.value)} className="w-full h-10 rounded-xl border-2 border-[#e8eaef] bg-white px-4 text-sm text-[#2d3142] focus:border-[#23ab7e] focus:outline-none transition-colors">
                {PLATFORMS.map((p) => <option key={p.value} value={p.value}>{p.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-sm font-bold text-[#8f96a3] mb-1.5 block">{t.websiteUrl}</label>
              <input value={comp.websiteUrl} onChange={(e) => updateCompetitor(idx, "websiteUrl", e.target.value)} placeholder="https://..." className="w-full h-10 rounded-xl border-2 border-[#e8eaef] bg-white px-4 text-sm text-[#2d3142] focus:border-[#23ab7e] focus:outline-none transition-colors" />
            </div>
            {competitors.length > 1 && (
              <button onClick={() => removeCompetitor(idx)} className="h-8 w-8 flex items-center justify-center rounded-xl text-[#8f96a3] hover:text-red-500 hover:bg-red-50 transition-colors"><Trash2 className="h-5 w-5" /></button>
            )}
          </div>
        ))}

        {error && <p className="text-sm text-red-600 flex items-center gap-2 font-bold"><AlertTriangle className="h-5 w-5" /> {error}</p>}

        {/* Language Toggle */}
        <div className="flex items-center justify-between p-3 rounded-xl border-2 border-[#e8eaef] bg-[#fafbfd]">
          <div>
            <h4 className="font-bold text-sm text-[#2d3142]">{locale === "ar" ? "لغة التحليل" : "Analysis Language"}</h4>
            <p className="text-sm text-[#8f96a3]">{locale === "ar" ? "اختر لغة نتائج التحليل والتقرير" : "Choose the language for the analysis output & PDF report"}</p>
          </div>
          <div className="flex rounded-xl border-2 border-[#e8eaef] overflow-hidden">
            <button
              onClick={() => setOutputLanguage("en")}
              className={`px-6 py-3 text-sm font-bold transition-all ${outputLanguage === "en" ? "bg-gradient-to-r from-[#23ab7e] to-[#8054b8] text-white" : "bg-white text-[#8f96a3] hover:bg-[#f4f6f8]"}`}
            >
              English
            </button>
            <button
              onClick={() => setOutputLanguage("ar")}
              className={`px-6 py-3 text-sm font-bold transition-all ${outputLanguage === "ar" ? "bg-gradient-to-r from-[#23ab7e] to-[#8054b8] text-white" : "bg-white text-[#8f96a3] hover:bg-[#f4f6f8]"}`}
            >
              العربية
            </button>
          </div>
      ...
      {analysisData && (
        <div className="space-y-6">
          {/* Tab bar */}
          <div className="flex rounded-xl border border-white/20 p-1.5 gap-1.5" style={{ background: "rgba(255,255,255,0.9)", backdropFilter: "blur(12px)" }}>
            {([
              ...
              <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`flex-1 flex items-center justify-center gap-2.5 py-4 rounded-xl text-sm font-bold transition-all ${activeTab === tab.key ? "bg-gradient-to-r from-[#23ab7e] to-[#8054b8] text-white shadow-lg" : "text-[#8f96a3] hover:bg-[#f4f6f8]"}`}>
          ...
          {activeTab === "overview" && (
            <div className="space-y-6">
              ...
              <SectionCard accentColor="linear-gradient(90deg, #23ab7e, #8054b8)">
                ...
                <p className="text-sm leading-relaxed text-[#505868] whitespace-pre-line">{analysisData.executiveSummary}</p>
              </SectionCard>
              ...
              <SectionCard accentColor="linear-gradient(90deg, #23ab7e, #10b981)">
                ...
                {analysisData.brandAssessment.marketPosition && (
                  <div className="p-3 rounded-xl bg-gradient-to-r from-[#23ab7e]/5 to-[#8054b8]/5 border border-[#e8eaef] mb-6">
                    <p className="text-sm text-[#505868] leading-relaxed font-medium">{analysisData.brandAssessment.marketPosition}</p>
                  </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-3 rounded-xl border border-[#23ab7e]/20 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(35,171,126,0.08), rgba(16,185,129,0.04))" }}>
                    <h4 className="font-bold text-xl text-[#23ab7e] mb-3 flex items-center gap-2"><TrendingUp className="h-5 w-5" /> {t.strengths}</h4>
                    <ul className="space-y-2.5">{analysisData.brandAssessment.strengths.map((s, i) => <li key={i} className="text-sm text-[#505868] leading-relaxed flex items-start gap-2"><span className="text-[#23ab7e] font-bold text-xl mt-0.5">+</span><span>{s}</span></li>)}</ul>
                  </div>
                  <div className="p-3 rounded-xl border border-red-200/40 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(239,68,68,0.08), rgba(248,113,113,0.04))" }}>
                    <h4 className="font-bold text-xl text-red-600 mb-3 flex items-center gap-2"><AlertTriangle className="h-5 w-5" /> {t.weaknesses}</h4>
                    <ul className="space-y-2.5">{analysisData.brandAssessment.weaknesses.map((w, i) => <li key={i} className="text-sm text-[#505868] leading-relaxed flex items-start gap-2"><span className="text-red-500 font-bold text-xl mt-0.5">-</span><span>{w}</span></li>)}</ul>
                  </div>
                  {analysisData.brandAssessment.opportunities && (
                    <div className="p-3 rounded-xl border border-[#8054b8]/20 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(128,84,184,0.08), rgba(167,139,250,0.04))" }}>
                      ...
                      <ul className="space-y-2.5">{analysisData.brandAssessment.opportunities.map((o, i) => <li key={i} className="text-sm text-[#505868] leading-relaxed flex items-start gap-2"><span className="text-[#8054b8] font-bold text-xl mt-0.5">★</span><span>{o}</span></li>)}</ul>
                    </div>
                  )}
                  {analysisData.brandAssessment.threats && (
                    <div className="p-3 rounded-xl border border-orange-200/40 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(245,158,11,0.08), rgba(251,146,60,0.04))" }}>
                      ...
                      <ul className="space-y-2.5">{analysisData.brandAssessment.threats.map((t2, i) => <li key={i} className="text-sm text-[#505868] leading-relaxed flex items-start gap-2"><span className="text-orange-500 font-bold text-xl mt-0.5">!</span><span>{t2}</span></li>)}</ul>
                    </div>
                  )}
                </div>
              </SectionCard>
              ...
              {/* Competitor Cards */}
              <div>
                <h3 className="flex items-center gap-3 text-xl font-extrabold text-[#2d3142] mb-5"><Swords className="h-6 w-6 text-[#23ab7e]" /> {t.competitorProfiles}</h3>
                <div className="space-y-6">
                  {analysisData.competitors.map((comp, i) => (
                    <SectionCard key={i} accentColor="linear-gradient(90deg, #8054b8, #a78bfa)">
                      <div className="flex items-start justify-between mb-5 flex-wrap gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-[#8f96a3] font-medium">{comp.handle} · {comp.platform}</p>
                          </div>
                        </div>
                      ...
                      {/* Key Insight Banner */}
                      <div className="p-3 rounded-xl bg-gradient-to-r from-[#23ab7e]/5 to-[#8054b8]/5 border-2 border-[#e8eaef] mb-6">
                        <p className="text-sm font-bold text-[#23ab7e] uppercase tracking-wider mb-1">{locale === "ar" ? "الرؤية الرئيسية" : "KEY INSIGHT"}</p>
                        <p className="text-sm text-[#2d3142] leading-7 font-medium">{comp.keyInsight}</p>
                      </div>

                      {comp.stealThisMove && (
                        <div className="p-3 rounded-xl bg-[#8054b8]/5 border-2 border-[#8054b8]/20 mb-6">
                          <p className="text-sm font-bold text-[#8054b8] uppercase tracking-wider mb-1 flex items-center gap-2"><Star className="h-4 w-4" /> {locale === "ar" ? "اسرق هذه الحركة" : "STEAL THIS MOVE"}</p>
                          <p className="text-sm text-[#2d3142] leading-7 font-medium">{comp.stealThisMove}</p>
                        </div>
                      )}
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {comp.companyOverview && (
                              <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                                <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><Building2 className="h-4 w-4" />{locale === "ar" ? "نظرة عامة على الشركة" : "Company Overview"}</p>
                                <p className="text-sm text-[#505868] leading-7">{comp.companyOverview}</p>
                              </div>
                            )}
                            {comp.productsAndServices && (
                              <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                                <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><Package className="h-4 w-4" />{locale === "ar" ? "المنتجات والخدمات" : "Products & Services"}</p>
                                <p className="text-sm text-[#505868] leading-7">{comp.productsAndServices}</p>
                              </div>
                            )}
                            {comp.targetMarket && (
                              <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                                <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><Users className="h-4 w-4" />{locale === "ar" ? "السوق المستهدف" : "Target Market"}</p>
                                <p className="text-sm text-[#505868] leading-7">{comp.targetMarket}</p>
                              </div>
                            )}
                            {comp.brandPositioning && (
                              <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                                <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><Crosshair className="h-4 w-4" />{locale === "ar" ? "تموضع العلامة التجارية" : "Brand Positioning"}</p>
                                <p className="text-sm text-[#505868] leading-7">{comp.brandPositioning}</p>
                              </div>
                            )}
                            {comp.websiteAnalysis && (
                              <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                                <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><Globe className="h-4 w-4" />{locale === "ar" ? "تحليل الموقع الإلكتروني" : "Website Analysis"}</p>
                                <p className="text-sm text-[#505868] leading-7">{comp.websiteAnalysis}</p>
                              </div>
                            )}
                            {comp.digitalPresence && (
                              <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                                <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><Eye className="h-4 w-4" />{locale === "ar" ? "الحضور الرقمي" : "Digital Presence"}</p>
                                <p className="text-sm text-[#505868] leading-7">{comp.digitalPresence}</p>
                              </div>
                            )}
                            {comp.pricingStrategy && (
                              <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                                <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><DollarSign className="h-4 w-4" />{locale === "ar" ? "استراتيجية التسعير" : "Pricing Strategy"}</p>
                                <p className="text-sm text-[#505868] leading-7">{comp.pricingStrategy}</p>
                              </div>
                            )}
                            {comp.customerReviews && (
                              <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                                <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><Star className="h-4 w-4" />{locale === "ar" ? "مراجعات العملاء" : "Customer Reviews"}</p>
                                <p className="text-sm text-[#505868] leading-7">{comp.customerReviews}</p>
                              </div>
                            )}
                            {comp.technologyStack && (
                              <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                                <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><Layers className="h-4 w-4" />{locale === "ar" ? "البنية التقنية" : "Technology Stack"}</p>
                                <p className="text-sm text-[#505868] leading-7">{comp.technologyStack}</p>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          ...
                          {[
                            ...
                            <div key={item.label} className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                              <p className="text-sm font-bold text-[#23ab7e] mb-1 flex items-center gap-1.5"><item.icon className="h-4 w-4" />{item.label}</p>
                              <p className="text-sm text-[#505868] leading-7">{item.value}</p>
                            </div>
                          ))}
                          {comp.captionStyle && (
                            <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                              <p className="text-sm font-bold text-[#23ab7e] mb-1">{locale === "ar" ? "أسلوب الكتابة" : "Caption Style"}</p>
                              <p className="text-sm text-[#505868] leading-7">{comp.captionStyle}</p>
                            </div>
                          )}
                        </div>
                        <div className="space-y-4">
                          {comp.audienceProfile && (
                            <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                              <p className="text-sm font-bold text-[#23ab7e] mb-1">{locale === "ar" ? "ملف الجمهور" : "Audience Profile"}</p>
                              <p className="text-sm text-[#505868] leading-7">{comp.audienceProfile}</p>
                            </div>
                          )}
                          {comp.contentCalendar && (
                            <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                              <p className="text-sm font-bold text-[#23ab7e] mb-1">{locale === "ar" ? "تقويم المحتوى" : "Content Calendar"}</p>
                              <p className="text-sm text-[#505868] leading-7">{comp.contentCalendar}</p>
                            </div>
                          )}
                          {comp.paidStrategy && (
                            <div className="p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                              <p className="text-sm font-bold text-[#23ab7e] mb-1">{locale === "ar" ? "الاستراتيجية المدفوعة" : "Paid Strategy"}</p>
                              <p className="text-sm text-[#505868] leading-7">{comp.paidStrategy}</p>
                            </div>
                          )}
                          <div className="p-3 rounded-xl border border-[#23ab7e]/20" style={{ background: "linear-gradient(135deg, rgba(35,171,126,0.08), rgba(16,185,129,0.04))" }}>
                            <p className="text-sm font-bold text-[#23ab7e] mb-2">{t.strengths}</p>
                            {comp.strengths.map((s, j) => <p key={j} className="text-sm text-[#505868] leading-7">+ {s}</p>)}
                          </div>
                          <div className="p-3 rounded-xl border border-red-200/40" style={{ background: "linear-gradient(135deg, rgba(239,68,68,0.08), rgba(248,113,113,0.04))" }}>
                            <p className="text-sm font-bold text-red-600 mb-2">{t.weaknesses}</p>
                            {comp.weakPoints.map((w, j) => <p key={j} className="text-sm text-[#505868] leading-7">- {w}</p>)}
                          </div>
                        </div>
                      </div>
                    </SectionCard>
              ...
              <SectionCard accentColor="linear-gradient(90deg, #0d9488, #06b6d4)">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={item.key} className="p-3 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                      <h4 className="font-black text-sm text-[#23ab7e] mb-2 flex items-center gap-2"><item.icon className="h-5 w-5" />{item.label}</h4>
                      <p className="text-sm text-[#505868] leading-7">{analysisData.saudiMarketInsights[item.key]}</p>
                    </div>
                  ))}
                  {analysisData.saudiMarketInsights.localOpportunities && (
                    <div className="p-3 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                      <h4 className="font-black text-sm text-[#23ab7e] mb-2 flex items-center gap-2"><Sparkles className="h-5 w-5" />{locale === "ar" ? "فرص محلية" : "Local Opportunities"}</h4>
                      <p className="text-sm text-[#505868] leading-7">{analysisData.saudiMarketInsights.localOpportunities}</p>
                    </div>
                  )}
                  {analysisData.saudiMarketInsights.ramadanStrategy && (
                    <div className="md:col-span-2 p-3 rounded-xl bg-gradient-to-r from-[#23ab7e]/5 to-[#8054b8]/5 border-2 border-[#e8eaef]">
                      <h4 className="font-black text-sm text-[#8054b8] mb-2 flex items-center gap-2"><Star className="h-5 w-5" />{locale === "ar" ? "استراتيجية رمضان" : "Ramadan Strategy"}</h4>
                      <p className="text-sm text-[#505868] leading-7">{analysisData.saudiMarketInsights.ramadanStrategy}</p>
                    </div>
                  )}
                </div>
              </SectionCard>
              ...
              {analysisData.industryAnalysis && (
                <SectionCard accentColor="linear-gradient(90deg, #6366f1, #8054b8)">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-3 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                      <h4 className="font-black text-sm text-[#23ab7e] mb-2 flex items-center gap-2"><BarChart3 className="h-5 w-5" />{locale === "ar" ? "نظرة عامة على السوق" : "Market Overview"}</h4>
                      <p className="text-sm text-[#505868] leading-7">{analysisData.industryAnalysis.marketOverview}</p>
                    </div>
                    <div className="p-3 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                      <h4 className="font-black text-sm text-[#23ab7e] mb-2 flex items-center gap-2"><Swords className="h-5 w-5" />{locale === "ar" ? "المشهد التنافسي" : "Competitive Landscape"}</h4>
                      <p className="text-sm text-[#505868] leading-7">{analysisData.industryAnalysis.competitiveLandscape}</p>
                    </div>
                    <div className="p-3 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                      <h4 className="font-black text-sm text-[#23ab7e] mb-2 flex items-center gap-2"><Users className="h-5 w-5" />{locale === "ar" ? "اتجاهات المستهلكين" : "Consumer Trends"}</h4>
                      <p className="text-sm text-[#505868] leading-7">{analysisData.industryAnalysis.consumerTrends}</p>
                    </div>
                    <div className="p-3 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                      <h4 className="font-black text-sm text-[#23ab7e] mb-2 flex items-center gap-2"><Rocket className="h-5 w-5" />{locale === "ar" ? "التوقعات المستقبلية" : "Future Outlook"}</h4>
                      <p className="text-sm text-[#505868] leading-7">{analysisData.industryAnalysis.futureOutlook}</p>
                    </div>
                  </div>
                </SectionCard>
              )}
          ...
          {activeTab === "compare" && (
            <div className="space-y-6">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b-2 border-[#e8eaef]">
                        <th className="text-start py-4 px-3 font-black text-sm text-[#8f96a3]">{t.category}</th>
                        <th className="text-start py-4 px-3 font-black text-sm text-[#23ab7e]">{t.yourBrand}</th>
                        {Object.keys(analysisData.comparisonMatrix.competitors).map((n) => <th key={n} className="text-start py-4 px-3 font-black text-sm text-[#2d3142]">{n}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {analysisData.comparisonMatrix.categories.map((cat, idx) => (
                        <tr key={cat} className="border-b border-[#e8eaef]/50 hover:bg-[#fafbfd] transition-colors">
                          <td className="py-4 px-3 font-bold text-sm text-[#8f96a3]">{cat}</td>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
                <div className="rounded-xl border-2 border-[#23ab7e] bg-[#f4f6f8] p-4 flex flex-col items-center">
                  <ScoreCircle score={analysisData.brandAssessment.overallScore} size={90} />
                  <p className="text-sm font-black text-[#23ab7e] mt-2">{t.yourBrand}</p>
                </div>
                {analysisData.competitors.map((comp) => (
                  <div key={comp.name} className="rounded-xl border-2 border-[#e8eaef] bg-white p-4 flex flex-col items-center">
                    <ScoreCircle score={comp.overallScore} size={90} />
                    <p className="text-sm font-black text-[#2d3142] mt-2">{comp.name}</p>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {analysisData.winningStrategy.quickWins.map((w, i) => (
                      <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-white border-2 border-[#e8eaef]">
                        ...
                        <p className="text-sm text-[#2d3142] leading-7">{w}</p>
                      </div>
                    ))}
                  </div>
                </SectionCard>
              ...
              <SectionCard accentColor="linear-gradient(90deg, #ef4444, #f59e0b)">
                ...
                <div className="space-y-4">
                  {analysisData.winningStrategy.immediate.map((a, i) => (
                    <div key={i} className="p-3 rounded-xl bg-red-50/50 border-2 border-red-100">
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <p className="text-sm text-[#2d3142] leading-7 flex-1 font-medium">{a.action}</p>
              ...
              <SectionCard accentColor="linear-gradient(90deg, #8054b8, #a78bfa)">
                ...
                <div className="space-y-4">
                  {analysisData.winningStrategy.shortTerm.map((a, i) => (
                    <div key={i} className="p-3 rounded-xl bg-yellow-50/30 border-2 border-yellow-100">
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <p className="text-sm text-[#2d3142] leading-7 flex-1 font-medium">{a.action}</p>
                        <div className="flex gap-2 shrink-0"><PriorityBadge priority={a.priority} /></div>
                      </div>
              ...
              <SectionCard accentColor="linear-gradient(90deg, #23ab7e, #10b981)">
                ...
                <div className="space-y-4">
                  {analysisData.winningStrategy.longTerm.map((a, i) => (
                    <div key={i} className="p-3 rounded-xl bg-[#f4f6f8] border-2 border-[#e8eaef]">
                      <div className="flex items-start justify-between gap-4 mb-2">
                        <p className="text-sm text-[#2d3142] leading-7 flex-1 font-medium">{a.action}</p>
                        <div className="flex gap-2 shrink-0"><PriorityBadge priority={a.priority} /></div>
                      </div>
                ...
                <SectionCard accentColor="linear-gradient(90deg, #ec4899, #8054b8)">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {analysisData.winningStrategy.contentSeries.map((s, i) => (
                      <div key={i} className="p-3 rounded-xl border-2 border-[#8054b8]/20 bg-[#8054b8]/5">
                        ...
                        <p className="text-sm text-[#505868] leading-7">{s.description}</p>
                      </div>
                    ))}
                  </div>
                </SectionCard>
                  ...
                  <div className="space-y-3">
                    {analysisData.winningStrategy.contentGaps.map((g, i) => (
                      <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                        ...
                        <p className="text-sm text-[#505868] leading-7">{g}</p>
                      </div>
                    ))}
                  </div>
                </SectionCard>
                  ...
                  <div className="space-y-3">
                    {analysisData.winningStrategy.differentiators.map((d, i) => (
                      <div key={i} className="flex items-start gap-3 p-4 rounded-xl bg-[#fafbfd] border border-[#e8eaef]">
                        ...
                        <p className="text-sm text-[#505868] leading-7">{d}</p>
                      </div>
                    ))}
                  </div>
                </SectionCard>
      ...
      {loading && (
        <div className="rounded-xl border border-white/20 bg-gradient-to-br from-[#f4f6f8] via-white to-[#fafbfd] flex flex-col items-center justify-center py-24 px-8 text-center relative overflow-hidden" style={{ backdropFilter: "blur(12px)" }}>
          {/* Floating 3D cubes */}
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            ...
            <div className="absolute bottom-[25%] left-[10%] w-12 h-9 rounded-lg bg-gradient-to-br from-[#06b6d4]/20 to-[#23ab7e]/20 border border-[#06b6d4]/10" style={{ animation: "float-cube 7s ease-in-out infinite 0.5s", transform: "rotate(60deg) perspective(200px) rotateX(30deg)" }} />
          ...
          <div className="relative mb-8">
            <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-full bg-gradient-to-br from-[#23ab7e] to-[#8054b8] flex items-center justify-center shadow-[0_8px_40px_rgba(35,171,126,0.35)]" style={{ animation: "pulse-glow 2s ease-in-out infinite" }}>
              <Swords className="h-8 w-8 sm:h-16 sm:w-16 text-white" style={{ animation: "float-cube 3s ease-in-out infinite" }} />
            </div>
          ...
          {/* Rotating fun quotes */}
          <p className="text-xl font-black text-[#2d3142] max-w-2xl leading-tight mb-4 transition-all duration-500" style={{ animation: "fade-text 4s ease-in-out infinite" }}>
            {funnyMessages[loadingMsg]}
          </p>
          <div className="flex items-center gap-3 mt-4">
            <Loader2 className="h-6 w-6 animate-spin text-[#23ab7e]" />
            <p className="text-sm text-[#8f96a3] font-bold">{t.analyzing}...</p>
          </div>
      ...
      {!analysisData && !loading && (
        <div className="rounded-xl border-2 border-dashed border-[#e8eaef]/50 flex flex-col items-center justify-center py-20 text-center" style={{ background: "rgba(255,255,255,0.7)", backdropFilter: "blur(12px)" }}>
          <Swords className="h-8 w-8 text-[#e8eaef] mb-5" />
          <p className="text-[#8f96a3] text-xl font-medium">{t.noAnalysisYet}</p>
        </div>
      )}
    </div>
  );
