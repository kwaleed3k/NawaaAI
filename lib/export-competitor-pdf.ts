...
function renderCoverPage(data: AnalysisData, companyName: string, locale: Locale): string {
  ...
  return `<div style="width:1120px;height:780px;background:linear-gradient(135deg,#23ab7e 0%,#1a8a64 30%,#8054b8 70%,#a45dd4 100%);color:white;font-family:'Outfit','Plus Jakarta Sans',sans-serif;box-sizing:border-box;position:relative;overflow:hidden;">
    ...
    <div style="position:relative;z-index:2;padding:56px 64px;height:100%;display:flex;flex-direction:column;justify-content:space-between;">
      <div style="display:flex;align-items:center;gap:16px;">
        <div style="width:52px;height:52px;background:rgba(255,255,255,0.15);border-radius:14px;border:1px solid rgba(255,255,255,0.2);display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:900;color:#a6ffea;">N</div>
        <span style="font-size:30px;font-weight:800;letter-spacing:-0.5px;">Nawaa AI</span>
      </div>

      <div>
        <p style="font-size:15px;font-weight:600;color:rgba(255,255,255,0.5);text-transform:uppercase;letter-spacing:4px;margin:0 0 16px;">Competitor Analysis Report</p>
        <p style="font-size:48px;font-weight:900;margin:0 0 10px;line-height:1.15;letter-spacing:-1px;">${companyName}</p>
        <p style="font-size:20px;color:rgba(255,255,255,0.65);margin:0 0 36px;">${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p>
        <div style="display:inline-block;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.18);border-radius:16px;padding:16px 28px;">
          <p style="margin:0 0 6px;font-size:11px;font-weight:700;color:#a6ffea;text-transform:uppercase;letter-spacing:2px;">Brand Score</p>
          <p style="margin:0;font-size:36px;font-weight:900;">${data.brandAssessment?.overallScore ?? "—"}<span style="font-size:16px;color:rgba(255,255,255,0.5);"> /100</span></p>
        </div>
      </div>

      <div style="display:flex;gap:40px;align-items:flex-end;">
        <div>
          <p style="margin:0;font-size:44px;font-weight:900;">${compCount}</p>
          <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.5);">Competitors</p>
        </div>
        <div>
          <p style="margin:0;font-size:44px;font-weight:900;">${data.comparisonMatrix?.categories?.length ?? 0}</p>
          <p style="margin:0;font-size:14px;color:rgba(255,255,255,0.5);">Categories</p>
        </div>
        <div style="flex:1;"></div>
        <div style="text-align:right;">
          <p style="margin:0;font-size:12px;color:rgba(255,255,255,0.4);">Generated by</p>
          <p style="margin:4px 0 0;font-size:15px;font-weight:700;color:#a6ffea;">Nawaa AI — Powered by AI</p>
          <p style="margin:4px 0 0;font-size:11px;color:rgba(255,255,255,0.3);">Vision 2030</p>
        </div>
      </div>
    </div>
  </div>`;
...
/* ═══ Executive Summary + Brand Assessment Page ═══ */
function renderSummaryPage(data: AnalysisData, companyName: string, locale: Locale): string {
...
async function addHtmlPage(pdf: jsPDF, html: string, isFirst: boolean) {
  ...
  if (target) void target.offsetHeight; // force layout
  await new Promise(r => setTimeout(r, 200));

  try {
    const canvas = await html2canvas(target || wrapper, {
      scale: 1.5,
      useCORS: true,
...
export async function exportCompetitorPdf(
  ...
  // Page 2: Executive Summary + Brand Assessment
  pageNum++;
  await addHtmlPage(pdf, renderSummaryPage(data, companyName, pdfLocale), false);
